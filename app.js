...
【延展模型】${steps.extend || ""}`;
  ...
  async callGuidedFeedback(steps) {
    ...
    const prompt = `这是我的觉察日记。请调用你课程中的具体模型、框架和金句，帮我看到我自己看不到的模式。
...
你可以尖锐，可以用你课程里的概念和金句，可以引用精神分析框架。不要泛泛而谈，要具体到我的文本。看到强迫性重复就直说，看到防御机制就命名它。`;
...
输出格式示例：["妈妈", "爸爸", "陈前"] 或 ["希里", "狸克"] 或 []`;
  ...
  async callAIFeedback(content) {
    const today = new Date().toLocaleDateString("zh-CN", { year: "numeric", month: "long", day: "numeric" });
    const prompt = `这是我的觉察日记（自由书写）。请用你（谢小树）的视角回应，调用你课程中的具体模型、框架和金句，帮我看清行为模式背后的核心需求。

你可以尖锐，可以直接引用课程里的概念和原文，可以命名防御机制，看到强迫性重复就直说。不要泛泛而谈，要具体到我的文本。
